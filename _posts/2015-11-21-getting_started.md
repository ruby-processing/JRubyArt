...
sudo pacman -S processing # installs processing-3.3.3  (community)
```
...
Install vanilla processing libraries from processing-3.3.3 ide (recomended audio, video)
...
Configure in `~/.jruby_art/config.yml`:-
```bash
PROCESSING_ROOT: "/home/tux/processing-3.3.3" # `substitute` user for `tux`
sketchbook_path: "/home/tux/sketchbook"
MAX_WATCH: 30
JRUBY: true
template: bare
...
Download processing-3.3.3 from the [official website][official] and install, prefer to install in say `~/processing-3.3.3`, that way you can keep processing-2.2.1 (or earlier version of processing), which you may find useful.  When you're done, make sure to take note of the directory you installed the app to complete the configuration.
...
Download processing-3.3.3 from the [official website][official] and install, prefer to install in say `~/processing-3.3.3`.  When you're done, make sure to take note of the directory you installed the app to complete the configuration see below.
