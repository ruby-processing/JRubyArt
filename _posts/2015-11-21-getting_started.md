...
In 2015, Martin Prout (aka monkstone) published the [JRubyArt gem][jrubyart], loosely based on the original ruby-processing, but since updated to use processing-3.3.6 and jruby-9.1.15.0 (ruby-2.2 syntax)
...
Why was ruby-processing not updated to use processing3.0+? The [major changes][changes] between processing-2.2.1 and processing-3.0 are not backward compatible. Furthermore since JRubyArt was designed to use jruby-9.0.0.0+ from the outset, it makes use of the more literate ruby-2.3 syntax (although the original ruby-processing may run with jruby-9.1.15.0, the examples and the ruby-processing library are all based on ruby-1.9.3 syntax).
...
sudo pacman -S jruby # installs jruby-9.1.14.0
sudo pacman -S processing # installs processing-3.3.6  (community)
```
...
Install vanilla processing libraries from processing-3.3.6 ide (recomended audio, video)
...
Use `update-alternatives` to install and maintain configuration eg for java:-
```bash
sudo update-alternatives --install /usr/bin/jruby jruby /opt/jruby-9.1.15.0/bin/jruby 100
sudo update-alternatives --config jruby # to configure if required
```

Configure in `~/.jruby_art/config.yml`:-
```bash
PROCESSING_ROOT: "/home/tux/processing-3.3.6" # `substitute` user for `tux`
sketchbook_path: "/home/tux/sketchbook"
MAX_WATCH: 30
JRUBY: true
template: bare
...
Download processing-3.3.6 from the [official website][official] and install, prefer to install in say `~/processing-3.3.6`, that way you can keep processing-2.2.1 (or earlier version of processing), which you may find useful.  When you're done, make sure to take note of the directory you installed the app to complete the configuration.
...
Download processing-3.3.6 from the [official website][official] and install, prefer to install in say `~/processing-3.3.6`.  When you're done, make sure to take note of the directory you installed the app to complete the configuration see below.
