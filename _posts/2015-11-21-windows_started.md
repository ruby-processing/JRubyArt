...
In 2015, Martin Prout (aka monkstone) published the [JRubyArt gem][jrubyart], loosely based on the original ruby-processing, but updated to use processing-3.3.5 and jruby-9.1.12.0 (ruby-2.3 syntax)
...
Setting JRubyArt for the first time, can seem a bit involved (especially if you are addicted to rvm or rbenv). The JRubyArt gem relies on jruby-9.1.12.0+, processing-3.3.5, and a handful of other dependencies. Here's how to get them all installed and working on Windows.
