...
Vanilla processing uses "Think Different Code on Mac" that relies on proprietary code such as `com/apple/eawt/QuitHandler`, this cannot be used with current versions of java > jdk8. This is one of reasons vanilla processing comes with its own jdk, and why Mac users need to use jdk8 with JRubyArt (even linux users have issues with `jdk11ea` no `awt` included).
